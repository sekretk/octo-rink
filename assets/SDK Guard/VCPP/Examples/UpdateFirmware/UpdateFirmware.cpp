// UpdateFirmware.cpp : Defines the entry point for the console application.
//
...
void DoCvtUpdateFW()
{
	...
	hr = ZG_UpdateCvtFirmware(&rOp, pData, nCount, UpdateFW_CB, NULL);
	free(pData);
	_tprintf(TEXT("\n"));

	if (!CheckZGError(hr, TEXT("ZG_UpdateCvtFirmware")))
		return;
	_tprintf(TEXT("Done.\n"));
}

BOOL CALLBACK CtrUpdateFW_CB(INT nPos, INT nMax, PVOID pUserData)
{
	_tprintf(TEXT("\rUpdate %3d%%..."), (nPos * 100) / nMax);
	return TRUE;
}
...
void DoCtrUpdateFW()
{
	...
	__try
	{
		...
		if (!CheckZGError(ZG_Cvt_UpdateCtrFirmware(hCvt, nCtrSn, pData, nCount, NULL, UpdateFW_CB, NULL, ZG_CTR_UNDEF), _T("ZG_Cvt_UpdateCtrFirmware")))
			return;
		_tprintf(TEXT("\n"));
		_tprintf(TEXT("Done.\n"));
	}
