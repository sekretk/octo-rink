...
void DoGuardFindCtrs()
{
	...
	g_nCtrCount = 0;
	if (!CheckZGError(ZG_Cvt_EnumControllers(g_hCvt, EnumCtrsCB, NULL, 0), _T("ZG_Cvt_EnumControllers")))
		return;
	if (g_nCtrCount > 0)
	{
		_tprintf(TEXT("--\n"));
...
void DoFindControllerMenu()
{
	...
	__try
	{
		...
		if (!CheckZGError(ZG_Cvt_Open(&g_hCvt, &rOp, NULL), _T("ZG_Cvt_Open")))
			return;
