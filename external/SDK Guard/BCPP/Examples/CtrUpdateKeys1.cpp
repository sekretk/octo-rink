...
const CtrAddr = 3;
LPCTSTR KeysCacheFileName_D = TEXT("ctr%d_keys.bin");	// Имя файла кэша ключей
...
void ShowKeys()
{
	...
	CZgKeyList aKeys(g_nOptRead);
	PZG_CTR_KEY pKey;
	ofstream f;
	String sCacheName;
	sCacheName.printf(KeysCacheFileName_D, g_nSn);

	f.open(sCacheName.c_str(), ios::out | ios::app | ios::binary | ios::trunc);
		...
		for (j = 0; j < nTop; j++) {
			if ((j % g_nOptRead) == 0) {
				...
				if (nCount > g_nOptRead)
					nCount = g_nOptRead;
				if (!CheckZGError(ZG_Ctr_ReadKeys(g_hCtr, j, &aKeys[0], nCount, NULL, NULL, i), TEXT("ZG_Ctr_ReadKeys")))
					return;
...
BOOL GetCtrList(CZgKeyList* pList)
{
	...
	if (GetFileAttributes(sCacheName.c_str()) != INVALID_FILE_ATTRIBUTES) {
		ifstream f;
		f.open(sCacheName.c_str(), ios::in | ios::binary);
		...
		for (int i = 0; i < g_nMaxBanks; i++) {
			...
			nPos = (i * g_nMaxKeys);
			if (nTop > 0) {
				if (!CheckZGError(ZG_Ctr_ReadKeys(g_hCtr, 0, &pList->at(nPos), nTop, ZgProcessCb, NULL, i), TEXT("ZG_Ctr_ReadKeys")))
					return FALSE;
...
BOOL GetNewList(CMyKeyList* pList)
{
	...
	while (f.good()) {
		getline(f, sLine);
		nSize = sLine.size() + 1;
		pToken = strtok_s((LPSTR)sLine.c_str(), &nSize, seps, &pNextToken);
		if ((pToken == NULL) || !ParseKeyNum(pToken, r.m_KeyNum))
			continue;
		r.m_nType = ZG_KEY_NORMAL;
		r.m_nAccess = 0xff;
		pToken = strtok_s(NULL, &nSize, seps, &pNextToken);
		if (pToken != NULL) {
			while (*pToken == ' ') ++pToken;
...
void SetCtrList(const CZgKeyList& oList, const CBits& oSync)
{
	...
	assert(oList.size() == (size_t)(g_nMaxBanks * g_nMaxKeys));
	BOOL fChanged = FALSE;
	...
	for (int i = 0; i < g_nMaxBanks; i++) {
		...
		while (nIdx < g_nMaxKeys) {
			...
			if (nWCnt > 0) {
				if (!CheckZGError(ZG_Ctr_WriteKeys(g_hCtr, nWIdx, const_cast<PZG_CTR_KEY>(&oList[nWPos]), nWCnt, NULL, NULL, i, FALSE), TEXT("ZG_Ctr_WriteKeys")))
					return;
...
void DoLoadKeysFromFile()
{
	...
	INT aNext[2] = {0};
	assert((int)_countof(aNext) >= g_nMaxBanks);
