...
LPCTSTR ModeSubEvStrs[] = {
	TEXT(""),
	TEXT("RS485_ALLOW"),		// Установка командой по сети
	TEXT("RS485_DENIED"),		// Отказано оператору по сети
	TEXT("Timezone start"),			// Началась временная зона
	TEXT("Timezone finish"),			// Окончилась временная зона
	TEXT("CARD_ALLOW"),			// Установка картой
	TEXT("CARD_DENIED")			// Отказано изменению картой
};
...
void ShowEvents(int nStart, int nCount)
{
	...
	while (i < nCount) {
		...
		nCnt = (nCount - i);
		if (nCnt > (int)_countof(aEvents))
			nCnt = (int)_countof(aEvents);

		if ((nIdx + nCnt) > g_nCtrMaxEvents)
			nCnt = (g_nCtrMaxEvents - nIdx);

		if (!CheckZGError(ZG_Ctr_ReadEvents(g_hCtr, nIdx, aEvents, nCnt, NULL, NULL), TEXT("ZG_Ctr_ReadEvents")))
			return;
		for (j = 0; j < nCnt; j++)
		{
			pEv = &aEvents[j];
...
{
	INT nWrIdx;
	if (!CheckZGError(ZG_Ctr_ReadEventIdxs(g_hCtr, &nWrIdx, NULL), TEXT("ZG_Ctr_ReadEventIdxs")))
		return;
	INT nNewCount;
...
void DoRestoreFactorySettings()
{
	...
	if (!CheckZGError(ZG_Ctr_WriteEventIdxs(g_hCtr, 0x3, 0, 0), TEXT("ZG_Ctr_WriteEventIdxs")))
		return;
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	__try {
		...
		INT nWrIdx, nRdIdx;
		if (!CheckZGError(ZG_Ctr_ReadEventIdxs(g_hCtr, &nWrIdx, &nRdIdx), TEXT("ZG_Ctr_ReadEventIdxs")))
			return 0;
