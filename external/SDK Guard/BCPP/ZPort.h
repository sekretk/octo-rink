...
#define ZP_IF_NO_MSG_LOOP		0x0001	// Приложение не имеет цикла обработки сообщений (Console or Service)
#define ZP_IF_LOG				0x0002	// Записывать лог

// Флаги для функции ZP_FindNotification
enum _ZP_DD_FLAGS
{
...
// Уведомления функции ZP_FindNotification (wParam - MsgCode, lParam - MsgParam)
...
typedef struct _ZP_PORT_INFO {
	...
	ZP_PI_FLAGS nFlags;			// Флаги порта (ZP_PF_...)
...
typedef struct _ZP_DDN_DEVICE_INFO {
	...
	INT nPortCount;
	UINT nChangeMask;				// Маска изменений (ZP_CIF_...)
} *PZP_DDN_DEVICE_INFO;
...
typedef struct _ZP_DD_NOTIFY_SETTINGS {
	UINT nNMask;			// Маска типов уведомлений ZP_NF_
...
// Настройки для детектора устройств
...
typedef INT ZP_STATUS DEPRECATED_ATTR;	// результат выполнения функции DLL
